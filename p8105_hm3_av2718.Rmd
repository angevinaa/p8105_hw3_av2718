...
```{r}
library(p8105.datasets) # load packages 
library(dplyr) 
library(rnoaa)
...
ggplot(max_temp, aes(x = year, y = tmax_mean, group = id)) +  # Plot average max temperature by year
...
In this graph, we observed that the temperature in the NYC Subway Stations generally fluctuates between -10 and 30 degrees. January is the coldest month; the average temperature ranges from -10 to 10 degrees. On the other hand, July is the hottest month, with an average temperature between 20 and 30 degrees. Interestingly, in July 1987, we observed an extreme outlier, with the average temperature in July dropping below 15 degrees. We can also perceive from the data structure a slight increase in the average temperature between 2005-2010. 
